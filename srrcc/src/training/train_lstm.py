...
def train_and_predict_lstm(
    ...
    try:
        ...
        if cfg.models.prediction.lstm.get('use_observation_features', False): # Use .get for safety
            obs_features = cfg.models.prediction.lstm.observation_features
    ...
    try:
        for split in ["train", "val", "test"]:
            ...
            for i, feature in enumerate(feature_names_ordered):
                path = low_dim_feature_paths.get(feature, {}).get(split)
                if path and os.path.exists(path):
                    has_split_data = True # 只要有一个特征有数据，就处理这个 split
                    logger.debug(f"  加载 {split} - 特征 '{feature}' from: {path}")
                    data = np.load(path).flatten() # 确保一维
                    ...
                    # 如果某个特征的某个 split 缺失，需要决定如何处理
                    # 选项1: 报错退出 (如果所有特征都必须存在)
                    # raise FileNotFoundError(f"未找到 {split} 数据 for 特征 '{feature}': {path}")
                    # 选项2: 填充或跳过 (复杂，不推荐)
                    # 选项3: 如果是 val/test 缺失，可以继续但不能评估该 split (当前逻辑)
                    if feature == target_field_name and split == 'train':
                         raise FileNotFoundError(f"训练数据缺失 for 目标特征 '{feature}': {path}")
                    logger.warning(f"  未找到 {split} 数据 for 特征 '{feature}' (路径: {path}). 如果是 val/test 可能 OK。")
                    # 为了保持形状一致，如果允许缺失，可能需要填充，但这里假设不填充，如果缺失则不处理该 split
                    if i == 0: # 如果第一个特征就缺失，这个split无效
                         has_split_data = False
                         break
                    else: # 如果非第一个特征缺失，也无法堆叠
                         logger.error(f"无法堆叠! 特征 '{feature}' 的 {split} 数据缺失。")
                         has_split_data = False
                         break # 停止处理这个 split
            ...
            if has_split_data and len(split_data_list) == num_input_features:
                 ...
                 logger.error(f"未能加载 {split} 的所有必需特征数据，跳过此分割。")
    ...
    while feature_names_ordered[target_feature_index] != target_field_name:
        logger.error(f"逻辑错误：目标特征未按预期放在第{target_feature_index}列！")
        target_feature_index += 1

    if target_feature_index >= len(feature_names_ordered):
        logger.error("逻辑错误：目标特征未在输入特征中找到！")
        return {}
