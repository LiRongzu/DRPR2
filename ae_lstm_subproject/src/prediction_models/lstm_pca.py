...
class LSTMPredictionModel(nn.Module):
    ...
    #     # x shape: (seq_len, input_size) for unbatched or (seq_len, batch, input_size) for batched
    #     device = x.device # Use device from input tensor

    #     # --- >>> 修改开始 <<< ---
    #     # 根据输入 x 的维度初始化隐藏状态 h0 和 c0
    #     if hx is None: # Only initialize if not provided
    #         if x.dim() == 2:
    #             # Unbatched input: (seq_len, input_size)
    #             # Initialize hidden state as 2D: (num_layers, hidden_size)
    #             h0 = torch.zeros(self.num_layers, self.hidden_size).to(device)
    #             c0 = torch.zeros(self.num_layers, self.hidden_size).to(device)
    #         elif x.dim() == 3:
    #             # Batched input: (seq_len, batch, input_size) since batch_first=False
    #             batch_size = x.size(1)
    ...
    #     # LSTM forward pass
    #     lstm_out, hidden_state = self.lstm(x, hx)
    #     # lstm_out shape: (seq_len, hidden_size) if unbatched, (seq_len, batch, hidden_size) if batched

    #     # --- >>> 修改开始 <<< ---
    #     # 取最后一个时间步的输出进行预测
    #     # Handle both batched and unbatched output
    #     if lstm_out.dim() == 2: # Unbatched output: (seq_len, hidden_size)
    #         last_time_step_out = lstm_out[-1, :] # Shape: (hidden_size)
    #     elif lstm_out.dim() == 3: # Batched output: (seq_len, batch, hidden_size)
    #         last_time_step_out = lstm_out[-1, :, :] # Shape: (batch, hidden_size)
    #     else:
    #          raise ValueError(f"Unexpected LSTM output dimension: {lstm_out.dim()}")

    #     out = self.fc(last_time_step_out)
    #     # --- >>> 修改结束 <<< ---

    #     # Return both prediction and the final hidden state for potential multi-step prediction
    #     return out # 只返回预测结果，迭代函数会处理下一步输入
