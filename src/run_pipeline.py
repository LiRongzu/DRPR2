...
def run_dimensionality_reduction(cfg: DictConfig) -> Dict[str, Any]:
    ...
    # --- SOM 逻辑 (保持不变) ---
        ...
        # ... (你原来的 SOM 逻辑，确保它最后也设置 results['success'] = True/False 和必要的路径)
        # ... (并确保 low_dim_data_paths 的结构是 {feature: {split: {'positions': path}}} )
        # 你的 SOM 逻辑看起来比较完整，这里省略以保持简洁
        ...
        if som_state_results and isinstance(som_state_results, dict):
            ...
            if bmu_paths_state:
                try:
                    ...
                    if not state_bmu_positions: raise ValueError("未能从状态 SOM 结果中提取有效的 BMU 位置路径。")
        ...
        if pred_method == 'hmm':
            ...
            if not observation_features:
                 logger.error("HMM 预测方法已选择，但未在配置中指定 observation_features。"); return results

        # 检查 LSTM 是否需要观测 (根据你的配置结构调整)
        elif pred_method == 'lstm' and cfg.model.prediction.get('lstm', {}).get('use_observation_features', False):
             needs_obs_som = True
             observation_features = list(cfg.model.prediction.get('lstm', {}).get('observation_features', []))
             if not observation_features:
                  logger.error("LSTM 配置了 use_observation_features=True 但未提供 observation_features。"); return results

        if needs_obs_som:
             obs_feature_name = "_".join(sorted(observation_features)) # 创建组合名称
             logger.info(f"训练观测 SOM ({obs_feature_name}) for {pred_method}...")
             # ... (临时修改 map_size 的逻辑, 调用训练函数 train_single/combined_feature_som, 恢复 map_size) ...
             # 假设训练成功并返回 som_obs_results
             ...
             logger.debug(f"观测 SOM 返回结果: {som_obs_results}") # DEBUG 级别
             ...
             if som_obs_results and isinstance(som_obs_results, dict):
                 ...
                 if bmu_paths_obs:
                     try:
                         ...
                         if not obs_bmu_positions: raise ValueError("未能从观测 SOM 结果中提取有效的 BMU 位置路径。")
        ...
        if low_dim_data_paths_for_pred:
            ...
            if needs_obs_som: required_features.append(obs_feature_name)
...
def run_prediction(cfg: DictConfig, dr_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if method == 'hmm':
        # ... (你原来的 HMM 逻辑) ...
        # 确保最后设置 results['success'] = True/False 和 results['predicted_low_dim_paths']
        if dr_method == 'som':
             ...
             if not state_som_model_path or not obs_som_model_path:
                  logger.error("SOM-HMM 模式缺少状态或观测 SOM 模型路径。"); return results

             # 定义 HMM 保存路径
             hmm_model_dir = os.path.join(config.paths.models_base_dir, f"{dr_method}_{method}") # e.g., models/som_hmm
             ...
             param_save_path = os.path.join(hmm_model_dir, hmm_param_filename)

             # 定义 HMM 预测输出的子目录和模式 (将在 HMM 内部使用)
             # HMM 内部需要知道保存在哪里，通常是 Hydra 目录
             # train_hmm_main 可能需要调整或接收输出目录参数
             # 假设 train_hmm_main 会自动使用 Hydra 的当前目录
             pred_state_pattern = "predicted_state_ranks_{split}.npy" # HMM 预测状态序列

             # --- 调用 HMM 训练 ---
             pred_results_hmm = train_hmm_main(
                 cfg,
                 state_som_model_path=state_som_model_path,
                 obs_som_model_path=obs_som_model_path,
                 param_save_path=param_save_path, # HMM 参数保存路径
                 predicted_state_save_pattern=pred_state_pattern # 传递模式给 HMM 内部使用
                 # 注意：train_hmm_main 可能需要 hydra_output_dir 参数来保存预测
             )

             # 处理 HMM 结果
             if pred_results_hmm and 'predicted_states_paths' in pred_results_hmm:
                  results['predicted_low_dim_paths'] = pred_results_hmm['predicted_states_paths'] # 状态路径用于重建
                  results['model_path'] = pred_results_hmm.get('standard_model_path') or pred_results_hmm.get('model_path') # HMM 模型参数路径
                  results['success'] = True
                  logger.info("HMM (SOM) 预测成功。")
             else:
                  logger.error("train_hmm_main 未返回预期结果或失败。")
                  results['success'] = False
        else:
