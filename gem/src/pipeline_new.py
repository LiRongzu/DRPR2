...
def run_dimensionality_reduction(cfg: DictConfig) -> Dict[str, Any]:
    ...
    target_feature = cfg.model.dimensionality_reduction.get("target_feature")
    # --- 获取处理后的数据目录 ---
    try:
        processed_dir = cfg.data.processed_dir
        if not processed_dir:
            raise ValueError("cfg.data.processed_dir is empty or not set.")
    except Exception as e:
        logger.error(f"无法从配置中获取 processed_dir: {e}")
        return {'success': False, 'method': method, 'target_feature': target_feature, 'error': 'Missing processed_dir'}
...
def run_reconstruction(cfg: DictConfig, dr_results: Dict[str, Any], pred_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if dr_method == 'som':
        ...
        for split, pred_path in predicted_low_dim_paths.items():
         ...
         scaler_info = None # 用于存储加载的 scaler 对象或参数字典
         scaler_type = None # 'pkl' or 'npy'
         ...
         try:
             ...
             if dr_method == 'pca':
                 ...
                 logger.error("Autoencoder 重建尚未完全实现。") # Placeholder
                 results['success'] = False; return results

             # --- 加载高维 Scaler (根据后缀) ---
             scaler_suffix = os.path.splitext(high_dim_scaler_path)[1].lower()
             if scaler_suffix == '.pkl':
                 scaler_info = joblib.load(high_dim_scaler_path)
                 scaler_type = 'pkl'
                 logger.info(f"已加载 Scaler 对象 (pkl): {high_dim_scaler_path}")
             elif scaler_suffix == '.npy':
                 scaler_content = np.load(high_dim_scaler_path, allow_pickle=True).item()
                 if isinstance(scaler_content, dict) and 'mean' in scaler_content and 'std' in scaler_content:
                     scaler_info = scaler_content
                     scaler_type = 'npy'
                     logger.info(f"已加载 Scaler 参数 (npy): {high_dim_scaler_path}")
                 else:
                     raise ValueError(f"加载的 .npy Scaler 文件格式不正确: {high_dim_scaler_path}")
             else:
                 raise ValueError(f"不支持的 Scaler 文件类型: {scaler_suffix}")
         ...
         for split, pred_path in predicted_low_dim_paths.items():
             ...
             try:
                 ...
                 if scaler_type == 'pkl':
                     ...
                     std_vec = scaler_info['std']
                     epsilon = 1e-8
                     # 检查维度匹配
                     if isinstance(mean_vec, np.ndarray) and mean_vec.ndim == 1 and len(mean_vec) == reconstructed_scaled.shape[1]:
                         reconstructed_high_dim = reconstructed_scaled * (std_vec + epsilon) + mean_vec
                     elif np.isscalar(mean_vec): # 处理全局 scaler
                         reconstructed_high_dim = reconstructed_scaled * (std_vec + epsilon) + mean_vec
                     else:
                         logger.error(f"  npy Scaler 维度 ({mean_vec.shape if isinstance(mean_vec, np.ndarray) else type(mean_vec)}) "
                                      f"与重建特征 ({reconstructed_scaled.shape[1]}) 不匹配。跳过反标准化。")
                         reconstructed_high_dim = reconstructed_scaled # 回退
                     logger.debug(f"  使用 npy Scaler 参数进行手动反标准化 ({split})")
                 else:
                     logger.error("  未知的 Scaler 类型，无法反标准化。")
                     reconstructed_high_dim = reconstructed_scaled # 回退
    ...
    for split, recon_path in reconstructed_paths.items():
        ...
        try:
            ...
            if boolean_mask is not None and target_spatial_shape is not None:
                 ...
                 # valid_spatial_points = boolean_mask # 直接使用布尔掩码
